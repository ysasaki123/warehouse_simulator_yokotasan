...
class MLPPlayer:
    def __init__(self):
        self.reg = MLPRegressor(hidden_layer_sizes=(10, 10), max_iter=100)
        ...
        for w in obs.warehouses:
            for product_id in w.product_repo:
                for amount in [20, 100]:
                    action = (w.id, product_id, amount)
                    possible_actions1.append(action)
                    features1.append(obs.get_feature(w.id, product_id) + [amount, 0])
                    possible_actions2.append((w.id, product_id, 0))
                    features2.append(obs.get_feature(w.id, product_id) + [amount, 1])
    ...
    def create_data(self):
        ...
        num_sample = min(5000, len(experiences))
        experiences = sample(experiences, num_sample)
        print('num_experience', len(experiences), num_sample)
        prev_state = np.array([s1 + [a[2], int(a[2] == 0)] for s1,a,r,s2 in experiences])
        next_state = np.array([s2 + [a[2], 0] for s1,a,r,s2 in experiences])
        next_state2 = np.array([s2 + [a[2], 1] for s1,a,r,s2 in experiences])
